...
// Task notes (Instructor listing page):
// - Display all instructors/providers from backend.
// - Navigate to provider profile page when user clicks a provider card.
...
export default function ProviderListPage() {
  ...
  useEffect(() => {
    // Initial provider directory load.
    getProviders()
        ...
        <section className="mb-stack-lg p-stack-lg rounded-xl bg-linear-to-br from-tertiary-container to-primary-container text-on-primary-container relative overflow-hidden">
          <div className="relative z-10 max-w-2xl">
            <h1 className="font-display text-display mb-4">
              Giảng viên & Nhà cung cấp
            </h1>
            <p className="font-body-lg text-body-lg opacity-90 leading-relaxed">
              Học hỏi từ các chuyên gia hàng đầu trong ngành. Đội ngũ giảng viên
              của chúng tôi luôn sẵn sàng chia sẻ kiến thức thực tế và kinh
              nghiệm quý báu để giúp bạn phát triển.
            </p>
          </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-gutter">
          {loading
            ? Array.from({ length: 10 }).map((_, i) => (
                <div
                  key={i}
                  className="flex flex-col items-center gap-3 p-6 rounded-2xl bg-surface border border-outline-variant/30 animate-pulse"
                >
